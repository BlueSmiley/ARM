...
start
	...
	LDR	R5, [R5]	; load array size
        MOV     R0,     R4      ;to pass array address as parameter
        
 
    
; sort program
        CMP     R5,     #0      ;to prevent 0 element arrays
        BEQ     stop
do
...
for
        MOV     R1,     R7      ; index -1
        LDR     R8,     [R4,    R1,     LSL     #2]     ;array [index-1]
        ...
        MOV     R2,     R7      ; index       
        LDR     R9,     [R4,    R2,     LSL     #2]     ;array[index]
        
        CMP     R8,     R9      ;compare array[index-1] to array[index]
        BLS     for     ;if [index -1] less than [index] repeat algorithm
...
;subroutine       
        ...
        b for
while
        CMP     R6,     #0      ;checks if any swaps have occured
        BNE     do	
stop	B	stop

; swap subroutine
; swaps the position of two indexes of a 2D array
; R0 = address of array start   R1 = index i    R2 = index j
;return void
swap
        STMFD   SP!,     {R4,R5,LR}
        LDR     R4,     [R0,    R1,     LSL     #2]    ;loads element 2
        LDR     R5,     [R0,    R2,     LSL     #2]     ;loads element 7
        
        STR     R5,     [R0,    R1,     LSL     #2]
        STR     R4,     [R0,    R2,     LSL     #2]     ;swaps elements 
        LDMFD   SP!,     {PC,R4,R5}                      ;end of subroutine
